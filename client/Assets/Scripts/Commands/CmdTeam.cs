...
namespace Assets.Scripts.Commands
{
    public class CmdTeam : Command
    {
        ...
        public CmdTeam(IActionListener actionListener, int action, object p)
        {
        ...
        public override void Paint(MyGraphics g)
        {
            ...
            if (player != null)
            {
                ...
                if (isOnline)
                {
                    myFontNameOnline.DrawString(g, name + " (lv" + Player.GetLevel(player.power) + ")", x + 90 + xBonus, yText, 0);
                }
                else
                {
                    myFontNameOffline.DrawString(g, name + " (lv" + Player.GetLevel(player.power) + ")", x + 90 + xBonus, yText, 0);
                }
