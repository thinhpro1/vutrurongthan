...
{
    public class CmdTeamMember : Command
    {
        ...
        public CmdTeamMember(IActionListener actionListener, int action, object p)
        {
        ...
        public override void Paint(MyGraphics g)
        {
            ...
            if (player != null)
            {
                ...
                if (isOnline)
                {
                    myFontNameOnline.DrawString(g, player.name + " (lv" + Player.GetLevel(player.power) + ")", x + 90 + xBonus, yText, 0);
                }
                else
                {
                    myFontNameOffline.DrawString(g, player.name + " (lv" + Player.GetLevel(player.power) + ")", x + 90 + xBonus, yText, 0);
                }
                myFontDescription.DrawString(g, description, x + 90 + xBonus, yText + hText + dis, 0);
            }
        }
    }
}
