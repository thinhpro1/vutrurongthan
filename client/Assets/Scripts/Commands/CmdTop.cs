...
namespace Assets.Scripts.Commands
{
    public class CmdTop : Command
    {
        ...
        public CmdTop(IActionListener actionListener, int action, object p)
        {
        ...
        public override void Paint(MyGraphics g)
        {
            ...
            if (player != null)
            {
                ...
                if (isOnline)
                {
                    myFontNameOnline.DrawString(g, "Top " + (topId + 1) + ": " + player.name, x + 90 + xBonus, yText, 0);
                }
                else
                {
                    myFontNameOffline.DrawString(g, "Top " + (topId + 1) + ": " + player.name, x + 90 + xBonus, yText, 0);
                }
