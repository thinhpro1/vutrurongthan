...
final class ResourceHandler {
    ...
    private static final int EFFECT_VERSION = 2;
    private static final int LEVEL_VERSION = 0;
    ...
    ResourceHandler(Session session, GameResources resources) {
    ...
    private void sendResourceManifest() throws IOException {
        int frameVersion = resources.frames().isEmpty()
                ? NOT_PROVIDED_VERSION : FRAME_VERSION;
        int levelVersion = resources.levels().isEmpty()
                ? NOT_PROVIDED_VERSION : LEVEL_VERSION;
        int effectVersion = resources.effects().isEmpty()
                ? NOT_PROVIDED_VERSION : EFFECT_VERSION;
    ...
    private void sendEffectResource() throws IOException {
        ...
        session.send(resourcePackets.effectResource(EFFECT_VERSION, effects));
    }
    ...
    private void sendLevelResource() throws IOException {
        ...
        session.send(resourcePackets.levelResource(LEVEL_VERSION, levels));
    }
    ...
    private void sendFrameResource() throws IOException {
        ...
        session.send(resourcePackets.frameResource(FRAME_VERSION, frames));
    }
